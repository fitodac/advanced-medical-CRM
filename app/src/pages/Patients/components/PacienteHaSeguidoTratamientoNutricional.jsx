...
import { Input, InputMask } from '../../../components/Ui'
...
export const PacienteHaSeguidoTratamientoNutricional = ({ context }) => {
	...
	const handleChange = (e) => {
		...
		if ('y' !== value)
			formContext.handleInputChange({
				target: {
					name: 'nt__followed_prescribed_nutritional_recommendation',
					value: '',
				},
			})
					...
					{fields.map(({ key, name, value, label }) => (
						<AlertMessage key={key} name={name} context={context}>
							<div className="flex gap-6 items-start">
								...
								{'hfnr__followed_prescribed_nutritional_recommendation_1' ===
									key &&
									'hfnr__followed_prescribed_nutritional_recommendation_1' ===
										input_visible && (
								...
								{'hfnr__followed_prescribed_nutritional_recommendation_2' ===
									key &&
									'hfnr__followed_prescribed_nutritional_recommendation_2' ===
										input_visible && (
										<div className="w-full relative -top-1">
											<Input
												name="hfnr__not_followed_prescribed_recommendation"
												context={context}
											/>
										</div>
									)}
