				...
				<section className="space-y-8">
					<div className="">
						<label className="input-checkbox">
							<span className="text-slate-600 text-base font-medium">
								{fields.bioimpedanciaElectrica.label}
							</span>
						</label>

						<div className="space-y-3">
							{fields.bioimpedanciaElectrica.options.map(
								({ key, name, label, append }) => (
									<AlertMessage key={key} name={name} context={context}>
										<div className="grid grid-cols-12 gap-x-4">
											<div className="col-span-4 flex items-center">
												<label
													className="text-sm leading-none font-normal w-full block select-none"
					...
					<div className="space-y-4">
						...
						<div className="space-y-3">
							...
							{fields.ecography.options.abdominal.map(
								({ key, name, label, append }) => (
									<AlertMessage key={key} name={name} context={context}>
										<div className="grid grid-cols-12 gap-x-4">
											<div className="col-span-4 flex items-center">
						...
						<div className="space-y-3">
							...
							{Object.keys(fields.ecography.options.muscular).map((e) => (
								<div key={e} className="space-y-3">
									{'axies' === e && <label className="">Ejes:</label>}
									{fields.ecography.options.muscular[e].map(
										({ key, name, label, append }) => (
											<AlertMessage key={key} name={name} context={context}>
												<div
													className={`grid grid-cols-12 gap-x-4 ${
														'adiposeTissue' === e && 'pt-3'
													}`}
												>
													<div className="col-span-4 flex items-center">
														<label className="text-sm leading-none font-normal w-full block select-none">
